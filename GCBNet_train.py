...
from comp.GCBNet_model import GCBNet
...
parser.add_argument('--model', type=str, default="GCBNet", help='choose the version of GAT.')
parser.add_argument('--dataset', type=str, default='LiDAR-UH', help='choose the dataset, name should be <setname>-<datasetname>-<loadtype>')
...
elif args.dataset.startswith('GII'):
    adj, features, labels, idx_train, idx_val, idx_test = load_citation(dataset_str=args.dataset.split('-')[1])
else:
    adj, features, labels, idx_train, idx_val, idx_test = load_ogb_data(args.dataset)


if args.model == "GCBNet":
    model = GCBNet(nfeat=features.shape[1], 
                nhid=args.hidden, 
